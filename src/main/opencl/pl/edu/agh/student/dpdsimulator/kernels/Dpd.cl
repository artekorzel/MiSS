...
double pairRandom(int dropletId, int neighbourId, int step) {
    ...
    return 2 * rand(&seed, 1) - 1;
    //return rand2(seed, seed2);
}

int calculateCellId(double3 position, double boxSizeX, double boxSizeY, double boxSizeZ, double cellRadius) {
...
double3 calculateForce(global double3* positions, global double3* velocities, global int* types, 
        global int* cells, global int* cellNeighbours, global double* virial, 
        constant PairParameters* pairParams, constant DropletParameters* dropletParams, 
        SimulationParameters simulationParams, int dropletId, int step, global int* states) {
    ...
    double cellRadius = simulationParams.cellRadius;
    ...
    double3 dropletVelocity = velocities[dropletId];    
    int dropletCellId = calculateCellId(dropletPosition, boxSizeX, boxSizeY, boxSizeZ, cellRadius);
    ...
    for(dropletCellNeighbourId = 0; dropletCellNeighbourId < 27; ++dropletCellNeighbourId) {
        ...
        for(j = 0, neighbourId = dropletNeighbours[j]; neighbourId >= 0; neighbourId = dropletNeighbours[++j]) {
            if(neighbourId != dropletId) {
                ...
                int pairId = dropletType * numberOfTypes + neighbourType;
                PairParameters pairParameters = pairParams[pairId];
                ...
                if(distanceValue < cutoffRadius) {
                    ...
                    float randNum = pairRandom(dropletId, neighbourId, step);

                    double conservativeForcePart = pi * weightRValue;
                    double dissipativeForcePart = -gamma * weightDValue
                            * dot(velocities[neighbourId] - dropletVelocity, normalizedPositionVector);
                    double randomForcePart = sigma * weightRValue * randNum;
                    
                    conservativeForce += conservativeForcePart * normalizedPositionVector;
                    dissipativeForce += dissipativeForcePart * normalizedPositionVector;                    
                    randomForce += randomForcePart * normalizedPositionVector;
                    
                    virial[pairId] += cutoffRadius * cutoffRadius 
                            * (conservativeForcePart + dissipativeForcePart + randomForcePart);
                    ++noOfNeighbours;
                }
            }
        }
...
kernel void calculateForces(global double3* positions, global double3* velocities,
        global double3* forces, global int* types, global int* cells, global int* cellNeighbours, global double* virial, 
        constant PairParameters* pairParams, constant DropletParameters* dropletParams, 
        constant SimulationParameters* simulationParameters, int step, global int* states) {
    ...
    forces[dropletId] = calculateForce(positions, velocities, types, cells, cellNeighbours, 
            virial, pairParams, dropletParams, simulationParams, dropletId, step, states);
}

kernel void calculateNewPositionsAndVelocities(global double3* positions, global double3* velocities,
        global double3* forces, global double* dropletsEnergy, global int* types,
        constant PairParameters* pairParams, constant DropletParameters* dropletParams, 
    ...
    double deltaTime = simulationParams.deltaTime;
    double3 dropletVelocity = velocities[dropletId];
    double dropletMass = dropletParams[types[dropletId]].mass;
    
    double3 newPosition = positions[dropletId] + dropletVelocity * deltaTime;
...
kernel void generateVelocities(global double3* velocities, global double3* forces, 
        global double* energy, global int* states, global int* types, 
...
kernel void calculateAverageVelocity(global double3* velocities, local double3* partialSums, 
...
kernel void fillCells(global int* cells, global double3* positions, 
    ...
    for(; dropletId < numberOfDroplets; ++dropletId) {
        ...
        int predictedCellId = calculateCellId(position, boxSizeX, boxSizeY, boxSizeZ, cellRadius);
